...
class DiskStorage:
    ...
    def get_item(self, key: str):
        if "token-code-verifier" in key:
            return st.query_params.get("cv")
        return None

    def set_item(self, key: str, value: str):
        if "token-code-verifier" in key:
            st.session_state["_last_cv"] = value
...
def _get_google_oauth_url() -> str:
    ...
    try:
        ...
        cv = st.session_state.get("_last_cv")
        if cv:
            import urllib.parse
            parsed = urllib.parse.urlparse(res.url)
            qs = urllib.parse.parse_qs(parsed.query)
            if 'redirect_to' in qs:
                base_redir = qs['redirect_to'][0]
                sep = "&" if "?" in base_redir else "?"
                new_redir = f"{base_redir}{sep}cv={cv}"
                qs['redirect_to'] = [new_redir]
...
def _render_google_button(key: str = "google_login"):
    """Google sign-in: rendered via components.html so it can reliably open a new tab outside the Streamlit iframe sandbox."""
